...
class Decompile:
    ...
    def make_shape_of_main(self): # 디컴파일 작업 맨 마지막 부분에 할 것
        ...
        self.c_code.insert(0, "int __cdecl main(int argc, const char argv, const char envp)\n{")        
        self.c_code.append("}")
    ...
    def decom_call(self):
        ...
        for command in self.assembly_command:
            ...
            #print function_name
            elif call_user.match(command):
                function_name = command.replace("call", "").strip()
                self.user_function.append(function_name) ## 함수정보를 따로 저장해야 user function 내부 디컴파일을 추가로 할 수 있습니다.
                
                mov_arg = re.compile('mov .*')
                arg_list = []
                mov_search_cnt = index-1
                while mov_arg.match(self.assembly_command[mov_search_cnt]): # call 명령어부터 거꾸로 올라가면서 push된 값들을 찾습니다.
                    argument = self.assembly_command[mov_search_cnt].replace("mov", "").strip()
                    
                    argument_list = list(argument)
                    
                    if argument_list[0] != 'e' :
                        break;
                    #if argument_list[0] = 'e'
                    #    print "hi"
                    # 만약 argument의 첫 시작이 e가 아니면, stop
